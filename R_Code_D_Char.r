setwd("C:/DDS/Semester2/AnalyticsProject/Git/Ppt2/With_Driver_charac")

library(apollo)
apollo_initialise()

database <- read.csv("Data_Char.csv",header = TRUE)


apollo_control=list(modelName  ="BaseModel",
                    modelDescr ="Simple MNL with ff",
                    indivID    ="Driver_tag",
                    workInLogs = TRUE,
                    outputDirectory = "output//Check"
                    #,panelData  = FALSE
)
...
forecast <- apollo_prediction(Base.0, apollo_probabilities, apollo_inputs, prediction_settings = list(runs=20))
